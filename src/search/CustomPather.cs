	...
	public class CustomPather {
		...
		**/
		...
		public CustomPather( TileMap searchMap, Func<TileCoord,TileCoord,int> distFunc=null, Func<TileCoord,bool> acceptTest=null ) {
			...
			if ( acceptTest == null ) {
				this.AcceptTest = DefaultAccept;
			} else {
				this.AcceptTest = acceptTest;
			}
			if ( distFunc == null ) {
				this.DistFunc = DefaultDist;
			} else {
		...
		**/
		...
		protected void QueueTile( TilePathNode prev, TileCoord coord ) {
			...
			if ( visited[coord.row, coord.col] != search_id && AcceptTest( coord ) ) {
				...
				TilePathNode node = new TilePathNode( prev, DistFunc( prev.coord, coord ), coord );
				Fringe.Push( node.distance + DistFunc( coord, destCoord ), node );
			...
			if ( coord.row > 0 ) {
				...
				if ( visited[nextCoord.row, nextCoord.col] != search_id ) {
					...
					if ( AcceptTest( nextCoord ) ) {
						...
						nextNode = new TilePathNode( prev, prev.distance + DistFunc(prev.coord, nextCoord), nextCoord );
						Fringe.Push( nextNode.distance + DistFunc( nextCoord, destCoord ), nextNode );
		...
		private bool DefaultAccept( TileCoord coord ) {
			return true;
		}
		 ...
		 */
		private int DefaultDist( TileCoord t1, TileCoord t2 ) {
