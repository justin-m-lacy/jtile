...
export class TilePathNode {

		// total distance to get to this location. (real, not estimated)
		public distance:number;

		public previous:TilePathNode;
		public coord:TileCoord;

		constructor( prev:TilePathNode, dist:number, tile:TileCoord ) {
			this.distance = dist;
			this.previous = prev;
			this.coord = tile;
		} //
...
export default class TilePather {
		...
		**/
		...
		constructor( searchMap:TileMap ) {

			this._map = searchMap;

			this.Fringe = new Heap<TilePathNode>( this._map.rows * this._map.cols );

			this.visited = [];

			this.AcceptTest = this.ApproveTile;
		 ...
		 */
		protected ApproveTile( coord:TileCoord ):boolean {
			return true;
		}
		...
		**/
		 ...
		 * dirCol,dirRow should be one of the cardinal directions.
		 */
		...
		protected QueueTile( prev:TilePathNode, coord:TileCoord ):void {
			...
			if ( this.visited[coord.row][coord.col] !== this.search_id && this.AcceptTest(coord) === true ) {
				...
				let node:TilePathNode = new TilePathNode( prev, prev.distance + 1, coord );
				this.Fringe.push( node.distance + this.TileDistance( coord, this.destCoord ), node );
			...
			if ( coord.row > 0 ) {
				...
				if ( this.visited[nextCoord.row][nextCoord.col] !== this.search_id ) {
					...
					if ( this.AcceptTest( nextCoord ) === true ) {
						...
						this.Fringe.push( nextNode.distance + this.TileDistance( nextCoord, this.destCoord ), nextNode );
		...
		public TileDistance( t1:TileCoord, t2:TileCoord ):number {
